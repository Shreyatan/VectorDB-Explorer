...
size_t estimateBruteMemory() {
    return store.size() * sizeof(VectorItem);
}

size_t estimateKDMemory() {
    return store.size() *
           (sizeof(KDNode) + sizeof(VectorItem));
}

size_t estimateHNSWMemory() {

    size_t edges = store.size() * 16;
...
double calculateRecall(
    const std::vector<std::pair<float,int>>& groundTruth,
    const std::vector<std::pair<float,int>>& approx)
{
    std::set<int> gt;
    ...
    for (auto& p : approx)
        if (gt.count(p.second))
            hit++;

    return groundTruth.empty()
        ? 0.0
        : (double)hit / groundTruth.size();
}

AnalyticsOut analytics(
    const std::vector<float>& q,
    int k,
    const std::string& metric)
{
    ...
    std::vector<std::pair<float,int>> bfRes;
    std::vector<std::pair<float,int>> kdRes;
    std::vector<std::pair<float,int>> hnswRes;

    long long bfUs = timeSearch(
        [&]{
    ...
    out.kdRecall = calculateRecall(bfRes, kdRes);
    out.hnswRecall = calculateRecall(bfRes, hnswRes);

    out.bruteMemory = estimateBruteMemory();
    out.kdMemory = estimateKDMemory();
    out.hnswMemory = estimateHNSWMemory();

    out.itemCount = store.size();
...
int main() {
    ...
    svr.Get("/analytics", [&](const httplib::Request& req,
                          httplib::Response& res)
{
    ...
    auto a = db.analytics(q, k, metric);
    ...
    ss << "\"latency\":{";
    ss << "\"bruteforce\":" << a.bfUs << ",";
    ss << "\"kdtree\":" << a.kdUs << ",";
    ss << "\"hnsw\":" << a.hnswUs;
    ss << "},";

    ss << "\"recall\":{";
    ss << "\"kdtree\":" << a.kdRecall << ",";
    ss << "\"hnsw\":" << a.hnswRecall;
    ss << "},";

    ss << "\"memory\":{";
    ss << "\"bruteforce\":" << a.bruteMemory << ",";
    ss << "\"kdtree\":" << a.kdMemory << ",";
    ss << "\"hnsw\":" << a.hnswMemory;
    ss << "},";

    ss << "\"itemCount\":" << a.itemCount;
